...
NSString *const BABYStringEncodingErrorDomain = @"com.google.BABYStringEncodingErrorDomain";
NSString *const BABYStringEncodingBadCharacterIndexKey = @"BABYStringEncodingBadCharacterIndexKey";
...
@implementation BABYStringEncoding
...
+ (id)hexStringEncoding {
  BABYStringEncoding *ret = [self stringEncodingWithString:
      @"0123456789ABCDEF"];
...
BABY_INLINE int lcm(int a, int b) {
  for (int aa = a, bb = b;;) {
    if (aa == bb)
      return aa;
    else if (aa < bb)
...
- (id)initWithString:(NSString *)string {
  if ((self = [super init])) {
    ...
    if (!charMapData_) {
      _BABYDevLog(@"Unable to convert string to ASCII");
      [self release];
      return nil;
    }
    ...
    if (length < 2 || length > 128 || length & (length - 1)) {
      _BABYDevLog(@"Length not a power of 2 between 2 and 128");
      [self release];
      return nil;
    }
    ...
    for (unsigned int i = 0; i < length; i++) {
      if (reverseCharMap_[(int)charMap_[i]] != kUnknownChar) {
        _BABYDevLog(@"Duplicate character at pos %d", i);
        [self release];
        return nil;
      }
...
- (void)ignoreCharacters:(NSString *)chars {
  ...
  while (*buf) {
    int c = *buf++;
    _BABYDevAssert(reverseCharMap_[c] == kUnknownChar,
...
- (NSString *)encode:(NSData *)inData error:(NSError **)error {
  ...
  if (!value) {
    if (error) {
      *error = [NSError errorWithDomain:BABYStringEncodingErrorDomain
                                   code:BABYStringEncodingErrorUnableToConverToAscii
                               userInfo:nil];
...
- (NSString *)encodeString:(NSString *)inString error:(NSError **)error {
  ...
  if (!data) {
    if (error) {
      *error = [NSError errorWithDomain:BABYStringEncodingErrorDomain
                                   code:BABYStringEncodingErrorUnableToConverToUTF8
                               userInfo:nil];
...
- (NSData *)decode:(NSString *)inString error:(NSError **)error {
        ...
        if (error) {
          NSDictionary *userInfo =
              [NSDictionary dictionaryWithObject:[NSNumber numberWithUnsignedInteger:i]
                                          forKey:BABYStringEncodingBadCharacterIndexKey];
          *error = [NSError errorWithDomain:BABYStringEncodingErrorDomain
                                       code:BABYStringEncodingErrorUnknownCharacter
                                   userInfo:userInfo];
        }
        return nil;
      }
          ...
          if (error) {
            NSDictionary *userInfo =
                [NSDictionary dictionaryWithObject:[NSNumber numberWithUnsignedInteger:i]
                                            forKey:BABYStringEncodingBadCharacterIndexKey];
            *error = [NSError errorWithDomain:BABYStringEncodingErrorDomain
                                         code:BABYStringEncodingErrorExpectedPadding
                                     userInfo:userInfo];
          }
          return nil;
        }
  ...
  if (bitsLeft && buffer & ((1 << bitsLeft) - 1)) {
    if (error) {
      *error = [NSError errorWithDomain:BABYStringEncodingErrorDomain
                                   code:BABYStringEncodingErrorIncompleteTrailingData
                               userInfo:nil];
