...
// Create a new, autoreleased BABYStringEncoding object with a standard encoding.
...
// Create a new, autoreleased BABYStringEncoding object with the given string,
// as described below.
+ (id)stringEncodingWithString:(NSString *)string;

// Initialize a new BABYStringEncoding object with the string.
//
...
// Only 7-bit ASCII characters are permitted in the string.
//
// e.g. If a BABYStringEncoder object has already been initialised with a set
...
FOUNDATION_EXPORT NSString *const BABYStringEncodingErrorDomain;
FOUNDATION_EXPORT NSString *const BABYStringEncodingBadCharacterIndexKey;  // NSNumber

typedef NS_ENUM(NSInteger, BABYStringEncodingError) {
  // Unable to convert a buffer to NSASCIIStringEncoding.
  BABYStringEncodingErrorUnableToConverToAscii = 1024,
  // Unable to convert a buffer to NSUTF8StringEncoding.
  BABYStringEncodingErrorUnableToConverToUTF8,
  // Encountered a bad character.
  // BABYStringEncodingBadCharacterIndexKey will have the index of the character.
  BABYStringEncodingErrorUnknownCharacter,
  // The data had a padding character in the middle of the data. Padding characters
  // can only be at the end.
  BABYStringEncodingErrorExpectedPadding,
  // There is unexpected data at the end of the data that could not be decoded.
  BABYStringEncodingErrorIncompleteTrailingData,
};
